...
const statusMap: Record<PortaalStatus, { label: string; bg: string; color: string; stap: number }> = {
  nieuw:               { label: "In wachtrij",         bg: "bg-[#1D4E89]/8",   color: "text-[#1D4E89]",  stap: 0 },
  ...
  transport_ingepland: { label: "Transport ingepland",  bg: "bg-blue-50",       color: "text-blue-700",    stap: 2 },
...
async function getAfroepen(klantNaam: string) {
  ...
  return (data ?? []).map((row) => ({
    ...row,
    io_nummer: (row.klant_io_nummers as { io_nummer: string } | null)?.io_nummer ?? null,
  }));
}
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <p className="text-xs font-semibold text-[#1F2328]/40 uppercase tracking-widest mb-1">Opvolging</p>
      ...
      {/* Status legenda */}
      <div className="bg-white border border-[#1F2328]/10 rounded-xl px-6 py-4 mb-6 flex items-center gap-0">
      ...
      {afroepen.length === 0 ? (
        <div className="bg-white border border-dashed border-[#1F2328]/15 rounded-xl p-12 text-center">
          ...
          <Link
            href="/portaal/afroep"
            className="inline-block px-6 py-2.5 bg-[#1D4E89] text-white text-sm font-semibold rounded-lg hover:bg-[#163d6e] transition-colors"
          >
        ...
        <div className="space-y-4">
          {afroepen.map((afr) => {
            ...
            return (
              <div key={afr.id} className="bg-white border border-[#1F2328]/10 rounded-xl overflow-hidden">
                {/* Header */}
                <div className="flex items-start justify-between px-6 py-4 border-b border-[#1F2328]/6">
                  <div className="flex items-start gap-4">
                    <div className="w-9 h-9 bg-[#F5F7FA] rounded-lg flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2.5 mb-1 flex-wrap">
                        <p className="text-sm font-bold text-[#1D4E89] font-mono">{afr.pallet_nr}</p>
                        <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${s.bg} ${s.color} border-current/20`}>
                          {s.label}
                        </span>
                        {afr.io_nummer && (
                          <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-[#1D4E89]/8 text-[#1D4E89] border border-[#1D4E89]/20 font-mono">
                            {afr.io_nummer}
                          </span>
                        )}
                        {afr.klant_ref && (
                          ...
                          <div key={label} className="flex items-center flex-1 last:flex-none">
                            <div className={`w-2.5 h-2.5 rounded-full border-2 shrink-0 transition-colors ${
                              i === 3
                                ...
                                : bereikt ? "bg-[#1D4E89] border-[#1D4E89]" : "bg-white border-[#1F2328]/20"
                            }`} />
                            {i < stapLabels.length - 1 && (
                              <div className={`flex-1 h-0.5 ${i < s.stap ? "bg-[#1D4E89]" : "bg-[#1F2328]/10"}`} />
                            )}
                          </div>
                        );
                      })}
