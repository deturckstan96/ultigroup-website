...
const statusMap: Record<PortaalStatus, { label: string; bg: string; color: string; stap: number }> = {
  nieuw:               { label: "In wachtrij",         bg: "bg-[#0C355F]/8",   color: "text-[#0C355F]",  stap: 0 },
  ...
  transport_ingepland: { label: "Transport ingepland",  bg: "bg-[#007EBE]/10",       color: "text-[#1C5A93]",    stap: 2 },
  ...
  return (
    <div>
      ...
      <div className="px-8 py-6 border-b border-[#D3DBE7] flex items-center justify-between" style={{ background: "#F4F1E8" }}>
        <div>
          <p className="text-xs font-semibold text-[#0C355F]/40 uppercase tracking-widest mb-1" style={{ fontFamily: "var(--font-display)" }}>Opvolging</p>
    ...
    <div className="p-8 max-w-5xl">
      ...
      {/* Status legenda */}
      <div className="bg-white border border-[#D3DBE7] px-6 py-4 mb-6 flex items-center gap-0">
      ...
      {afroepen.length === 0 ? (
        <div className="bg-white border border-dashed border-[#1F2328]/15 rounded-xl p-12 text-center">
          ...
          <Link
            href="/portaal/afroep"
            className="inline-block px-6 py-2.5 bg-[#0C355F] text-white text-sm font-semibold rounded-lg hover:bg-[#10457A] transition-colors"
          >
        ...
        <div className="space-y-4">
          {afroepen.map((afr) => {
            ...
            return (
              <div key={afr.id} className="bg-white border border-[#D3DBE7] overflow-hidden">
                {/* Header */}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2.5 mb-1 flex-wrap">
                        <p className="text-sm font-bold text-[#0C355F] font-mono">{afr.pallet_nr}</p>
                        <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${s.bg} ${s.color} border-current/20`}>
                          {s.label}
                        </span>
                        {afr.io_nummer && (
                          <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-[#0C355F]/8 text-[#0C355F] border border-[#0C355F]/20 font-mono">
                            {afr.io_nummer}
                          </span>
                        )}
                        {afr.klant_ref && (
                          ...
                          <div key={label} className="flex items-center flex-1 last:flex-none">
                            <div className={`w-2.5 h-2.5 rounded-full border-2 shrink-0 transition-colors ${
                              i === 3
                                ...
                                : bereikt ? "bg-[#0C355F] border-[#0C355F]" : "bg-white border-[#1F2328]/20"
                            }`} />
                            {i < stapLabels.length - 1 && (
                              <div className={`flex-1 h-0.5 ${i < s.stap ? "bg-[#0C355F]" : "bg-[#1F2328]/10"}`} />
                            )}
                          </div>
                        );
                      })}
