...
const statusMap: Record<SalesStatus, { label: string; bg: string; color: string }> = {
  Draft:                { label: "Ingediend",      bg: "bg-[#0C355F]/8",  color: "text-[#0C355F]"   },
  Confirmed:            { label: "Bevestigd",      bg: "bg-amber-50",     color: "text-amber-700"    },
  "Partially Delivered":{ label: "In behandeling", bg: "bg-[#007EBE]/10", color: "text-[#1C5A93]"   },
  ...
  return (
    <div>
      ...
      <div className="px-8 py-6 border-b border-[#D3DBE7]" style={{ background: "#F4F1E8" }}>
        <p className="text-xs font-semibold text-[#0C355F]/40 uppercase tracking-widest mb-1" style={{ fontFamily: "var(--font-display)" }}>Welkom terug</p>
        <h1 className="text-2xl font-bold text-[#0C355F]" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.02em" }}>{klantNaam ?? "Portaal"}</h1>
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border border-[#D3DBE7] p-5" style={{ borderRadius: 0 }}>
          ...
          <p className="text-3xl font-bold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>{totaleStuks.toLocaleString("nl-BE")}</p>
        ...
        <div className="bg-white border border-[#D3DBE7] p-5" style={{ borderRadius: 0 }}>
          ...
          <p className="text-3xl font-bold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>{aantalOpen}</p>
        ...
        <div className="bg-white border border-[#D3DBE7] p-5" style={{ borderRadius: 0 }}>
          ...
          {laatsteLevering ? (
            <>
              <p className="text-lg font-bold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>
        ...
        <div className={`border p-5 ${laagsteStock ? "bg-amber-50 border-amber-200" : "border-[#D3DBE7]"}`} style={{ borderRadius: 0, background: laagsteStock ? undefined : "rgba(90,140,74,0.08)" }}>
          <p className={`text-xs font-semibold uppercase tracking-widest mb-2 ${laagsteStock ? "text-amber-600" : "text-[#1C5A93]"}`} style={{ fontFamily: "var(--font-display)" }}>
            Stock status
          </p>
          <p className={`text-lg font-bold ${laagsteStock ? "text-amber-700" : "text-[#0C355F]"}`} style={{ fontFamily: "var(--font-display)" }}>
            {laagsteStock ? "Let op" : "OK"}
          </p>
          <p className={`text-xs mt-1 ${laagsteStock ? "text-amber-600" : "text-[#1C5A93]"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Stock samenvatting */}
        <div className="bg-white border border-[#D3DBE7] p-6" style={{ borderRadius: 0 }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>Mijn stock</h2>
            <Link href="/portaal/stock" className="text-xs font-medium hover:underline" style={{ color: "#007EBE" }}>Alles bekijken →</Link>
          </div>
                ...
                return (
                  <div key={nr}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className={`font-semibold ${laag ? "text-amber-600" : "text-[#0C355F]"}`}>
                        {art.qty.toLocaleString("nl-BE")} st
                      </span>
                    </div>
                    <div className="h-1.5 overflow-hidden" style={{ background: "#EDE8D8" }}>
                      <div className={`h-full ${laag ? "bg-amber-400" : "bg-[#0C355F]"}`}
                        style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                );
        ...
        <div className="bg-white border border-[#D3DBE7] p-6" style={{ borderRadius: 0 }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>Recente afroepen</h2>
            <Link href="/portaal/afroepen" className="text-xs font-medium hover:underline" style={{ color: "#007EBE" }}>Alles bekijken →</Link>
          </div>
          {recenteAfroepen.length === 0 ? (
            ...
            <div className="space-y-3 mb-4">
              {recenteAfroepen.slice(0, 3).map((a) => {
                ...
                return (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b border-[#D3DBE7] last:border-0">
                    <div>
                      <p className="text-sm font-medium text-[#0C355F] font-mono">{a.id}</p>
          ...
          <Link href="/portaal/afroep"
            className="flex items-center justify-center gap-1.5 w-full py-2.5 border border-dashed border-[#0C355F]/20 text-sm transition-colors hover:border-[#007EBE] hover:text-[#007EBE]"
      ...
      {/* Laatste levering */}
      {laatsteLevering && (
        <div className="bg-white border border-[#D3DBE7] p-6 mt-6" style={{ borderRadius: 0 }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-[#0C355F]" style={{ fontFamily: "var(--font-display)" }}>Laatste levering</h2>
            <Link href="/portaal/leveringen" className="text-xs font-medium hover:underline" style={{ color: "#007EBE" }}>Volledige historiek →</Link>
          </div>
          <div className="flex items-start gap-4 flex-wrap">
            <div className="min-w-[120px]">
              <p className="text-xs text-[#1F2328]/40 mb-0.5">Datum</p>
              <p className="text-sm font-semibold text-[#0C355F]">
            ...
            <div className="min-w-[120px]">
              ...
              <p className="text-sm font-semibold text-[#0C355F] font-mono">{laatsteLevering.delivery_id}</p>
            </div>
            {laatsteLevering.ref && (
              <div className="min-w-[120px]">
                <p className="text-xs text-[#1F2328]/40 mb-0.5">Klant ref</p>
                <p className="text-sm font-semibold text-[#0C355F]">{laatsteLevering.ref}</p>
              </div>
            )}
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {leveringRegels.map((r, i) => (
                  <span key={i} className="text-xs text-[#0C355F]/70 px-3 py-1 font-mono" style={{ background: "#EDE8D8" }}>
